...
### 3.0.2 - July 01, 2020
* Fixed the issue that caused a configuration change to be skipped as a part of refresh when a change is detected in a key with `refreshAll` set to `true` and all attempts to retrieve the updated settings from App Configuration fail.
